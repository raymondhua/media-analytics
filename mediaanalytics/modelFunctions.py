...
def similarWords(year, wordToFind):
...
def similarity(year, word1, word2):
...
def specificWord(year, wordString):
    ...
    for word in wordArray:
        getWordInfo(year, word, data['word'])
    data['code'] = 1
    return data
...
def compareBetweenTwoYears(yearFrom, yearTo, word):
    # Initializes the data dictionary with one key, word initialized to empty list
    data = {}
...
def analogies(year, positive, negative):
    ...
    try: 
        ...
        for x, y in similarWordArray: 
            data['similarWords'].append({'x': negative[0], 'y': positive[0], 'z': positive[1], 'word': x})
            score = score + 1
            # I'm only returning the most top similar word in the similar word array
            if score > 0:
                break
    except:
...
def tnseImage(model, items):
    tsne_df = makeTSNE(model, items)
...
def getTSNEImage(year, words):
    model = openModel(year)
    items = convertToList(words)
    return tnseImage(model, items)